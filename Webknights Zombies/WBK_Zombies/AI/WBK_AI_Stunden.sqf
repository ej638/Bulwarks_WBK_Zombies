...
_unitWithSword forceAddUniform "WBK_SpecialInfected_Screamer";
_unitWithSword setSpeaker "NoVoice";
_unitWithSword disableConversation true;
_unitWithSword setUnitPos "UP";
_unitWithSword setVariable ["WBK_SynthHP",WBK_Zombies_ScreamerHP,true];
...
_unitWithSword addEventHandler ["Killed", {
	...
	_this spawn {
		...
		if (!(isNil {_zombie getVariable "WBK_Zombie_CustomSounds"})) then {
			...
			[_zombie, selectRandom ["screamer_death_1","screamer_death_2"], 80, 6] execVM "\WebKnight_StarWars_Mechanic\createSoundGlobal.sqf";
		};
	};
}];
...
_actFr = [{
	...
	switch true do {
		case ((isNil {_mutant getVariable "CanMakeClue"}) and (isNil {_mutant getVariable "CanScream"}) and (count _ins != 0) and ((_en distance _mutant) > 2.2) and ((_en distance _mutant) <= WBK_Zombies_ScreamerDistance) and !(gestureState _mutant in ["wbk_zombie_attack_left","wbk_zombie_attack_right","wbk_zombie_attack_scream"])): {
			_mutant setVariable ["CanMakeClue",false];
			[_mutant, selectRandom ["screamer_knowsAbout_1","screamer_knowsAbout_2"], 300, 6] execVM "\WebKnight_StarWars_Mechanic\createSoundGlobal.sqf";
			_mutant spawn {uiSleep WBK_Zombies_ScreamerCooldown; _this setVariable ["CanMakeClue",nil];};
		};
		case ((isNil {_mutant getVariable "CanScream"}) and (count _ins == 0) and ((_en distance _mutant) > 2.2) and ((_en distance _mutant) <= WBK_Zombies_ScreamerDistance) and !(gestureState _mutant in ["wbk_zombie_attack_left","wbk_zombie_attack_right","wbk_zombie_attack_scream"])): {
			_mutant setVariable ["CanScream",false];
			_mutant playActionNow "WBK_Zombie_attack_Scream";
			[_mutant, selectRandom ["screamer_scream_1","screamer_scream_2"], 500, 6] execVM "\WebKnight_StarWars_Mechanic\createSoundGlobal.sqf";
			_mutant spawn {
				{
					if (alive _x && _x != _this) then {
						if !(isNil {_x getVariable "WBK_AI_ISZombie"}) then {
								_x doMove (getPosATL _this);
						}else{
							_insScream = lineIntersectsSurfaces [
								eyePos _this,
								eyePos _x,
								_this,
								_x,
							...
							if (count _insScream == 0) then {
								[_x, {
									if (currentWeapon _this == primaryWeapon _this && currentWeapon _this != "") then {
										_this playActionNow "WBK_Zombie_attack_Scream_Victim_Armed";
									}else{
										_this playActionNow "WBK_Zombie_attack_Scream_Victim_UnArmed";
									};
									_this forceWalk true;
									[_this,0.25,_this] spawn WBK_CreateDamage;
									if (isPlayer _this) then {
										enableCamShake true;
										addCamShake [5, 5, 25];
										playSound "Screamer_AffectPlayer";
									...
									if !(alive _this) exitWith {
										[_this, selectRandom ["decapetadet_sound_1","decapetadet_sound_2"], 100, 4] execVM "\WebKnight_StarWars_Mechanic\createSoundGlobal.sqf";
										_this call WBK_Zombies_CreateBloodParticle;
										_this unlinkItem hmd _this;
										removeGoggles _this;
										removeHeadgear _this;
										[_this, "WBK_DecapatedHead_Zombies_Normal"] remoteExec ["setFace",0];
									};
									uiSleep 3;
				...
				} forEach nearestObjects [_this,["MAN"],(WBK_Zombies_ScreamerDistance) + 10];
				uiSleep WBK_Zombies_ScreamerCooldown; 
				_this setVariable ["CanScream",nil];
			};
		};
		case (((_en distance _mutant) <= 2.2) and (count _ins == 0) and !(gestureState _mutant in ["wbk_zombie_attack_left","wbk_zombie_attack_right"])): {
			_mutant spawn {
				_this playActionNow selectRandom ["wbk_zombie_attack_left","wbk_zombie_attack_right"];
				[_this, selectRandom ["axe_punch_empty_1","axe_punch_empty_2","axe_punch_empty_3"], 50, 6] execVM "\WebKnight_StarWars_Mechanic\createSoundGlobal.sqf";
				uiSleep 0.25;
				if !(gestureState _this in ["wbk_zombie_attack_left","wbk_zombie_attack_right"]) exitWith {};
				[_this,0.5,2,false] call WBK_ZombieAttackDamage;
			};
		};
		default {};
	};
	...
	switch true do {
		case !(isNil {_unit getVariable "WBK_IsUnitLocked"}): {
			...
			[_unit, selectRandom ((_unit getVariable "WBK_Zombie_CustomSounds") select 0), 70] call CBA_fnc_GlobalSay3D;
				...
				[_unit, selectRandom ["screamer_aggro_1","screamer_aggro_2","screamer_aggro_3","screamer_aggro_4"], 70] call CBA_fnc_GlobalSay3D;
			};
		};
		default {
			_nearEnemy = _unit findNearestEnemy _unit; 
			...
			switch true do {
				...
				case ((isNull _nearEnemy) or !(alive _nearEnemy) or !(alive _unit) or (_unit distance _nearEnemy >= WBK_Zombies_SpecialInfected_MoveDistanceLimit)): {
					if (!(isNil {_unit getVariable "WBK_Zombie_CustomSounds"})) then {
						[_unit, selectRandom ((_unit getVariable "WBK_Zombie_CustomSounds") select 0), 70, "wbk_horde_calm_01", false] call WBK_ZombiePlayIdleSounds;
					}else{
						[_unit, selectRandom ["screamer_idle_1","screamer_idle_2","screamer_idle_3","screamer_idle_4"], 70, "wbk_horde_calm_01", false] call WBK_ZombiePlayIdleSounds;
					};
				};
				case !(isNil {_unit getVariable "WBK_AI_LastKnownLoc"}): {
					...
					if (!(isNil {_unit getVariable "WBK_Zombie_CustomSounds"})) then {
						[_unit, selectRandom ((_unit getVariable "WBK_Zombie_CustomSounds") select 1), 70, "wbk_horde_01", true] call WBK_ZombiePlayIdleSounds;
					}else{
						[_unit, selectRandom ["screamer_aggro_1","screamer_aggro_2","screamer_aggro_3","screamer_aggro_4"], 70, "wbk_horde_01", true] call WBK_ZombiePlayIdleSounds;
					};
				};
				case (!(isNull _nearEnemy) && (alive _nearEnemy)): {
					_unit doMove (getPosATLVisual _nearEnemy);
